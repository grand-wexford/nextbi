...
import { STATIC_REQUEST_URL,C  } from '../../constants';
...
class App extends Component {
  ...
  getComponent = (component = 'NotFound') => CORE_COMPONENTS[component];
      ...
      <Route
        render={({ location }) =>
          <NavigationDrawer
            ...
            navItems={this.MENU.map((item,i) => <NavLink {...item} location={location} key={i} />)}>
            <Switch key={location.key}>
              {this.getRouteItems().map((item,i) => <Route exact={item.route === '/' ? true : item.exact} path={item.route} location={location} component={this.getComponent(item.component)} key={i} />)}
