...
function cons(funcName, a) {
    /* eslint-disable no-console */
    funcName += a ? ': ' : '';
    a = a || {};

    console.log(funcName, Array.prototype.map.call(a, function (elem) {
        return elem;
    }).join(", "));
...
function graphInit(width, height, myjson, uuid, origOffsetX, origOffsetY, origScaleCoef, vertex1, vertex2) {
    ...
    function dragendSvg(d, i) {
        cons('dragendSvg', arguments);
        gvp.graphViewPort('notifyAndRedraw');
    }

    function rescale() {
        cons('rescale', arguments);
    ...
    function dragstart(d, i) {
        cons('dragstart', arguments);
        d3.event.sourceEvent.stopPropagation();
    }

    function dragmove(d, i) {
        cons('dragmove', arguments);
...
(function ($) {
    ...
    var pMethods = {
        updateUserScaleCoef: function (scaleCoef) {
            cons('pMethods.updateUserScaleCoef', arguments);
            var gvp = this;
            var value;
        ...
        doZoom: function (scaleCoef) {
            cons('pMethods.doZoom', arguments);
            var gvp = this;
        ...
        updateScaleCoef: function (scaleCoef) {
            cons('pMethods.updateScaleCoef', arguments);
            var gvp = this;
    ...
    var methods = {
        ...
        rescaleAndUpdate: function (deltaX, deltaY, scale) {
            ...
            cons('methods.rescaleAndUpdate', arguments);
        ...
        updateParams: function (origOffsetX, origOffsetY, origScaleCoef) {
            cons('methods.updateParams', arguments);
            var gvp = this;
        ...
        zoomOut: function () {
            cons('methods.zoomOut', arguments);
            var gvp = this;
        ...
        zoomCustom: function (scaleCoef) {
            cons('methods.zoomCustom', arguments);
            var gvp = this;
        ...
        updateZoom: function () {
            cons('methods.updateZoom', arguments);
        ...
        acceptFilters: function () {
            cons('methods.acceptFilters', arguments);
            var gvp = this;
        ...
        clickOverElements: function () {
            cons('methods.clickOverElements', arguments);
            var gvp = this;
        ...
        notifyAndRedraw: function () {
            cons('methods.notifyAndRedraw', arguments);
            var gvp = this;
            methods.notify.apply(gvp);
            methods.redraw.apply(gvp);
        },
        notify: function () {
            cons('methods.notify', arguments);
            var gvp = this;
            var params = {
                width: gvp.width(),
                height: gvp.height(),
